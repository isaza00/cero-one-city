...
| Map size/shape | **[s2.1 "the district"]** 1v1: **192×192**; FFA 3–4: **224×224**; start cores a fifth of the way in from the corners; symmetric; 1 unit per tile; buildings have footprints (core 2×2). Movement doubled (s1.2), vision ~1.6×, turret range 6, walking_tower range 8, camp guards aggro 7/leash 11. Terrain is a dead suburb on a street grid (3-tile avenues, `cero_engine/layout.py`, recomputed from the seed by the client): blocks of houses beside the avenues, traffic jams on them, groves and rock outcrops between — all `blocked`; collapsed houses and jam debris are `rubble`. Mapgen still scatters scaled center veins + expansion veins + up to 6/12 camps |
...
| **[as-built] Ruleset s2.1 "the district" (2026-09-07)** | 192×192 / 224×224 maps, starts at size/5 (goldens regenerated). Structured terrain instead of noise blobs: a seed-derived street grid shared by engine and client (`layout.py` ↔ `web/src/three/roads.ts`, pure 32-bit integer hash), neighbourhoods of 4–6×3–5-tile houses beside the avenues (one in five collapsed → rubble), rush-hour jams of wrecked cars on the avenues, groves and outcrops off them, loose rubble with skeletons and car parts. Every prop stands on an impassable tile, so units path around houses, cars, trees and rock. 3D: flat ground with low hills and a faint tile grid, asphalt with worn lane paint that vanishes where the street is gone, human-scale props (car 2.7 tiles, floor 1.9, tree 4), static props merged per 16×16 chunk and rebuilt per chunk when a tile changes. Third pass: bomb craters (seed-derived, only on open ground, walkable), burnt and burning houses, concrete ruins cut in half, charred and flaming trees, burnt farms with dead cattle, darker lighting and palette. Verifier: `engine/tools/dump_map.py` + `web/tools/verify-district.mjs` |
